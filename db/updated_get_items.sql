CREATE OR REPLACE FUNCTION get_items(IN plist text, IN locations int[], IN current_user int)
  RETURNS TABLE(store character varying, product character varying, brand character varying, category character varying, id int, price real, size real, unit character varying, ppu real, vegan boolean, p_un boolean, b_un boolean, histlow real, taste int, nutrition int, notes character varying) AS
$BODY$
BEGIN
RETURN QUERY
SELECT store.name AS store, product.name AS product, brand.name AS brand, category.name AS category, item.id AS id, item.price AS price, item.size AS size, unit.name AS unit, item.ppu AS ppu, item.vegan AS vegan, item.product_unavailable AS p_un,  item.brand_unavailable AS b_un, item.histlow AS histlow, user_item.taste AS taste, user_item.nutrition AS nutrition, user_item.notes AS notes
FROM item
...
LEFT JOIN user_item ON user_item.item_id = item.id AND user_item.user_id = current_user
...
ORDER BY store.name, category.name, product.name; END; $BODY$
